...
class Apocalypse(poc_grid.Grid):
    ...
    def compute_distance_field(self, entity_type):
        ...
        while len(boundary) > 0:
            ...
            for neighbor_cell in visited.four_neighbors(current_cell[0], current_cell[1]):
                if visited.is_empty(neighbor_cell[0], neighbor_cell[1]) and (self.is_empty(neighbor_cell[0],neighbor_cell[1]) or neighbor_cell in self._zombie_list or neighbor_cell in self._human_list):
    ...
    def move_humans(self, zombie_distance_field):
        ...
        for h_index, human in enumerate(self._human_list):
            ...
            # Only consider moves that are not blocked by an obstacle
            possible_moves = []
                       
            for move in moves:
                if self.is_empty(move[0],move[1]) or move in self._zombie_list or move in self._human_list:
